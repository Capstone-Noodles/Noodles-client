...
const PostForm = ({ navigation }) => {
  ...
  const uploadButton = useCallback(async () => {
    try {
      ...
      // setContent(splitContents[0].trim());
      const hashtagWordList = [];
      for (let i = 1; i < splitContents.length; i++) {
        hashtagWordList.push(splitContents[i].trim());
      }
      ...
      form.append("hashtagWordList", hashtagWordList.toString());
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: "white", height: "100%" }}>
      ...
      >
        ...
        >
          <TouchableOpacity
            ...
            style={{ padding: upload ? 0 : 30 }}
          >
            <Entypo
              name={upload ? "" : "camera"}
              style={{ fontSize: 30, color: "#484848" }}
            />
          </TouchableOpacity>
      ...
      >
        ...
        >
          <Text style={{ fontSize: 15, color: "#484848" }}>업로드</Text>
        </TouchableOpacity>
      </View>
