...
const EditProfile = ({ navigation, route } = () => {}) => {
  ...
  const [nickname, setNickname] = useState(
    route.params.userNickname.userNickname
  );
  const [description, setDiscription] = useState(
    route.params.userStateMessage.userStateMessage
  );
  const [userImg, setUserImg] = useState(
    route.params.profileImage.profileImage
  );
  ...
  const pressSubmitBtn = () => {
    ...
    if (modified) {
      ...
      form.append("imageFile", { uri: image, name: filename });
    } else {
      form.append("imageFile", { uri: userImg, name: userImg });
    }

    // console.log(form);
    axios
      .patch("http://133.186.228.218:8080/mypage/profile", form, {
        headers: {
    ...
    <View style={{ flex: 1, backgroundColor: "white" }}>
      ...
      <View style={{ margin: 25 }}>
        <View style={{ alignItems: "center", paddingBottom: 20 }}>
          ...
          >
            <ImageBackground
              source={modified ? { uri: image } : { uri: userImg }}
            ...
            >
              ...
              >
                <TouchableOpacity
                  onPress={
                    upload ? delete_image : () => setModalVisible(!modalVisible)
                  }
                >
                  <Ionic
                    name={upload ? "trash-bin" : "camera"}
                    size={35}
                    color="#ffbfbf"
                    style={{
                      opacity: 0.7,
